...
static bool chatpad_device_enabled(void)
{
    ...
    return (status != RpcKeyboardChatpadStatusStopped) && (status != RpcKeyboardChatpadStatusError);
}
            ...
            do
                {
                    ...
                    if (rpc_keyboard)
                        {
                            if (!storage_file_open(file, CHATPAD_DATA_FILE, FSAM_READ, FSOM_OPEN_EXISTING)) break;
                            char buffer[256];
                            for (int i = 0; i < 26; i++)
                                {
    ...
    if (index == 1)
        {
            ...
            if (rpc_keyboard)
                {
                    char* macro_text = rpc_keyboard_get_macro(rpc_keyboard, 'a' + app->setting_macro_index);
                    if (macro_text)
                        {
            ...
            text_input_set_result_callback(app->text_input, chatpad_setting_macro_updated, app, app->temp_buffer, app->temp_buffer_size, clear_previous_text);

            // Pressing the BACK button will reload the configure screen.
            view_set_previous_callback(text_input_get_view(app->text_input), chatpad_navigation_configure_callback);
...
static void chatpad_view_chatpad_enter_callback(void* context)
{
    ...
    app->timer = furi_timer_alloc(chatpad_view_chatpad_timer_callback, FuriTimerTypePeriodic, context);
    ...
    if (rpc_keyboard_pubsub != NULL)
        {
            with_view_model(app->view_chatpad, ChatpadModel * model, { model->keyboard_subscription = furi_pubsub_subscribe(rpc_keyboard_pubsub, chatpad_view_chatpad_keyboard_callback, app); }, false);
        }
    furi_record_close(RECORD_RPC_KEYBOARD);
}
...
static bool chatpad_view_chatpad_custom_event_callback(uint32_t event, void* context)
{
    ...
    switch (event)
        ...
        case ChatpadEventIdRedrawScreen:
            ...
            {
                bool redraw = true;
                with_view_model(app->view_chatpad, ChatpadModel * model, { model->chatpad_status = chatpad_device_status(); }, redraw);
                return true;
            }
...
static ChatpadApp* chatpad_app_alloc()
{
    ...
    submenu_add_item(app->submenu, "Config", ChatpadSubmenuIndexConfigure, chatpad_submenu_callback, app);
    submenu_add_item(app->submenu, "Chatpad", ChatpadSubmenuIndexChatpad, chatpad_submenu_callback, app);
    submenu_add_item(app->submenu, "About", ChatpadSubmenuIndexAbout, chatpad_submenu_callback, app);
    view_set_previous_callback(submenu_get_view(app->submenu), chatpad_navigation_exit_callback);
    view_dispatcher_add_view(app->view_dispatcher, ChatpadViewSubmenu, submenu_get_view(app->submenu));
    view_dispatcher_switch_to_view(app->view_dispatcher, ChatpadViewSubmenu);

    app->text_input = text_input_alloc();
    view_dispatcher_add_view(app->view_dispatcher, ChatpadViewTextInput, text_input_get_view(app->text_input));
    ...
    variable_item_set_current_value_text(item, setting_chatpad_enabled_names[setting_chatpad_enabled_index]);

    app->setting_macro = variable_item_list_add(app->variable_item_list_config, "Macro", 26, chatpad_setting_macro_change, app);
    variable_item_set_current_value_text(app->setting_macro, "A");
    variable_item_list_set_enter_callback(app->variable_item_list_config, chatpad_setting_item_clicked, app);
    app->setting_macro_index = 0;

    view_set_previous_callback(variable_item_list_get_view(app->variable_item_list_config), chatpad_navigation_submenu_callback);
    view_dispatcher_add_view(app->view_dispatcher, ChatpadViewConfigure, variable_item_list_get_view(app->variable_item_list_config));
    ...
    widget_add_text_scroll_element(app->widget_about, 0, 0, 128, 64, "Xbox 360 Chatpad!\nauthor: @codeallnight\nhttps://discord.com/invite/NsjCvqwPAd\nhttps://youtube.com/@MrDerekJamison");
    view_set_previous_callback(widget_get_view(app->widget_about), chatpad_navigation_submenu_callback);
    view_dispatcher_add_view(app->view_dispatcher, ChatpadViewAbout, widget_get_view(app->widget_about));
